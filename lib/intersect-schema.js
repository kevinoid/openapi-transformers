...
export default function intersectSchema(schema1, schema2) {
  ...
  if (schema1.title !== undefined || schema2.title !== undefined) {
    const t1 = schema1.title === undefined ? 'Untitled'
      : schema1.title.includes(' ') ? `(${schema1.title})`
        : schema1.title;
    const t2 = schema2.title === undefined ? 'Untitled'
      : schema2.title.includes(' ') ? `(${schema2.title})`
        : schema2.title;
    intersection.title = `Intersection of ${t1} and ${t2}`;
  }

  if (schema1.description !== undefined || schema2.description !== undefined) {
    const d1 = schema1.description === undefined ? 'Undescribed'
      : schema1.description.includes(' ') ? `(${schema1.description})`
        : schema1.description;
    const d2 = schema2.description === undefined ? 'Undescribed'
      : schema2.description.includes(' ') ? `(${schema2.description})`
        : schema2.description;
    intersection.description = `Intersection of ${d1} and ${d2}`;
  }
...
intersectKeywordByName.description = false;
...
intersectKeywordByName.title = false;
